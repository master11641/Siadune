...
class TableUser extends SqfEntityTableBase {
  TableUser() {
    ...
    fields = [
      ...
      SqfEntityFieldBase('password', DbType.text, isNotNull: false),
...
class User {
  ...
  User.withFields(this.firstName, this.lastName, this.username, this.password,
      this.token, this.isDeleted) {
    _setDefaultValues();
  }
  User.withId(this.id, this.firstName, this.lastName, this.username,
      this.password, this.token, this.isDeleted) {
    _setDefaultValues();
  }
  ...
  List<dynamic> toArgs() {
    return [firstName, lastName, username, password, token, isDeleted];
  }

  List<dynamic> toArgsWithIds() {
    return [id, firstName, lastName, username, password, token, isDeleted];
  }
  ...
  static Future<List<dynamic>> saveAll(List<User> users) async {
    // final results = _mnUser.saveAll('INSERT OR REPLACE INTO user (id,firstName, lastName, username, password, token,isDeleted)  VALUES (?,?,?,?,?,?,?)',users);
  ...
  Future<int> upsert() async {
    try {
      if (await _mnUser.rawInsert(
              'INSERT OR REPLACE INTO user (id,firstName, lastName, username, password, token,isDeleted)  VALUES (?,?,?,?,?,?,?)',
              [
                id,
                firstName,
                lastName,
